...
test('defaults to paper mode and redundant Robinhood Chain RPC transports', () => {
...
test('parses redundant RPC and websocket providers', () => {
  const config = loadConfig({ RPC_URLS: ' https://rpc-a.example ,https://rpc-b.example ', WS_URLS: ' wss://ws-a.example,wss://ws-b.example ', PAPER_CAPITAL_USD: '25000', MAX_RPC_LATENCY_MS: '500', MAX_BLOCK_LAG: '2' });
  ...
  assert.equal(config.paperCapitalUsd, 25_000); assert.equal(config.maxRpcLatencyMs, 500); assert.equal(config.maxBlockLag, 2); assert.equal(config.chainId, CHAIN.id);
});
