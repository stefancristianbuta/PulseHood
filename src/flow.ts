...
export class FlowEngine {
  ...
  process(input: FlowInput): FlowSnapshot | undefined {
    ...
    return {
      token,
      windowStartMs: state.startMs,
      buyVolumeUsd: state.buyVolumeUsd,
      sellVolumeUsd: state.sellVolumeUsd,
      volumeUsd,
      buyPressurePct,
      buyCount: state.buyCount,
      sellCount: state.sellCount,
      uniqueBuyers: state.buyers.size,
      uniqueSellers: state.sellers.size,
      uniqueBuyerDeltaPct,
  ...
  clear(token?: `0x${string}`): void {
    if (token === undefined) this.windows.clear();
    else this.windows.delete(token.toLowerCase());
  }
}
