...
export class RadarIngest {
  ...
  private async seedPools(toBlock: bigint): Promise<void> {
    ...
    try {
      ...
      const tokenPairs = await Promise.all(pairs.map(async (pair) => {
        try {
          const [token0, token1] = await Promise.all([
            this.client.readContract({ address: pair as `0x${string}`, abi: V2_PAIR_ABI, functionName: 'token0' }),
            this.client.readContract({ address: pair as `0x${string}`, abi: V2_PAIR_ABI, functionName: 'token1' }),
          ]);
          return { pair: pair as `0x${string}`, token0: token0 as `0x${string}`, token1: token1 as `0x${string}` };
      ...
      console.log(JSON.stringify({ event: 'pool_seed_v2', totalPairs: total, inspectedPairs: pairs.length, quotePairs: quotePairs.length }));
    } catch (error) {
    ...
    try {
      ...
      for (const log of created) {
        const args = log.args as { pool?: `0x${string}`; token0?: `0x${string}`; token1?: `0x${string}` };
        if (args.pool && args.token0 && args.token1 && isQuote(args.token0) !== isQuote(args.token1)) {
          this.addPool(args.pool, 'uniswap-v3');
          this.poolTokens.set(args.pool.toLowerCase(), { token0: args.token0, token1: args.token1 });
        }
      }
      console.log(JSON.stringify({ event: 'pool_seed_v3', discoveredPools: created.length, quotePools: created.filter((log) => { const a = log.args as { token0?: `0x${string}`; token1?: `0x${string}` }; return !!a.token0 && !!a.token1 && isQuote(a.token0) !== isQuote(a.token1); }).length }));
    } catch (error) {
