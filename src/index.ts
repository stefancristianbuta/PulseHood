...
qs('#stopBtn').onclick=async()=>{const r=await fetch('/api/control/stop',{method:'POST'});alert((await r.json()).message)};qs('#sellBtn').onclick=async()=>{const r=await fetch('/api/control/sell-all',{method:'POST'});alert((await r.json()).message)};refresh();loadDex();setInterval(refresh,5000);
</script></body></html>`;
...
async function main(): Promise<void> {
  ...
  const json = (res: import('node:http').ServerResponse, status: number, body: unknown): void => { res.writeHead(status, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' }); res.end(JSON.stringify(body)); };
